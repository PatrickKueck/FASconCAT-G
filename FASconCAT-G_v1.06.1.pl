...
# updated on 22nd february  , 2022 by jgl 			-> FcC-G v1.05.1 Resolved command conflict: 'Replace missing sequences with' set to (-g); 'Preface' set to (-x); some minor character corrections to comments
...
# updated on 12th june		, 2024 by patrick kueck -> FcC-G_v1.06	 Reconstruction of single infile handling and data storage across the whole source code for better RAM usage 
####################################################### START #######################################################################################
...
sub start{
	...
	for my $infile ( sort @input_files ){
		...
		unless ( $aref_parameter_fil->[0]  eq 'Supermatrix' ){
			...
			&print_out (
						...
						\$infile,							# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
						\%taxa_presampled,					# key: taxon name; value : number of occurence in infiles								-> IN (defined) / OUT (unchanged)
						\'datafile'							# flag for file printing																-> IN (defined) / OUT (unchanged)
			) ;
		}
		...
		unless ( $aref_parameter_fil->[0]  eq 'Convert' ){
			...
			&concatenate (
						...
						\%taxa_presampled,				# key: taxon name; value : number of occurence in infiles								-> IN (defined) / OUT (unchanged)
						\$structure_seq,				# defined if structure sequence present in infiles										-> IN (defined) / OUT (unchanged)
						\%concstring_of_taxon,			# key: taxon; value: concatenated string												-> IN (defined) / OUT (changed)
						\$infile,						# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
	...
	## If additional parameter info option set to yes and defined structure sequence found in infiles
	## extract structure sequence info like loop and stem positions; N of states...
	if ( ( $aref_parameter_inf->[0] eq 'YES' ) && ( $structure_seq ) && ( $aref_parameter_fil->[0]  =~ /Supermatrix/ ) ){

		&structure_handling (

						\%concstring_of_taxon,			# key: taxon; value: corresponding sequences											-> IN (defined) / OUT (unchanged)
						\"supermatrix",					# supermatrix 																			-> IN (defined) / OUT (unchanged)
						\%hoh_info_of_infile_of_type,	# key1: infilename, key2: type of info (e.g. 'seqlength'); info type associated value	-> IN (defined) / OUT (changed)
		) ;
	}
	############################################################ END STRUCTURE SEQUENCE ANALYSIS
	...
	## Count additional info about number of character states for each file sorted by missing data, ambiguity, indel event, and informative state
	## if additional info options is set to 'YES'
	if ( ( $aref_parameter_inf->[0] eq 'YES' ) && ( $aref_parameter_fil->[0]  =~ /Supermatrix/ ) ){
	...
	if ( $aref_parameter_fil->[0]  =~ /Supermatrix/ ){
		...
		$hoh_info_of_infile_of_type{supermatrix}{seqend}-- ;
		##############################
		
		##############################
		## assign number of supermatrix including taxon number
		$hoh_info_of_infile_of_type{supermatrix}{Ntaxa} = keys %concstring_of_taxon ;
		##############################
		...
		&print_out (
						...
						\@$aref_parameter_mis,				# Replacement code of missing gene sequences 											-> IN (defined) / OUT (unchanged)
						\$structure_seq,					# defined if structure sequence present in infiles										-> IN (defined) / OUT (unchanged)
						\'supermatrix',						# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
						\%taxa_presampled,					# key: taxon name; value : number of occurence in infiles								-> IN (defined) / OUT (unchanged)
						\'datafile'							# flag for file printing																-> IN (defined) / OUT (unchanged)
		) ;
	}
	...
	&print_out (
						...
						\@$aref_parameter_mis,				# Replacement code of missing gene sequences 											-> IN (defined) / OUT (unchanged)
						\$structure_seq,					# defined if structure sequence present in infiles										-> IN (defined) / OUT (unchanged)
						\'none',							# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
						\%taxa_presampled,					# key: taxon name; value : number of occurence in infiles								-> IN (defined) / OUT (unchanged)
						\'none'								# flag for file printing																-> IN (defined) / OUT (unchanged)
	) ;
	############################################################ END PRINT OUT OF GENERAL INFO
	...
	sub start_prottest{
		...
		my $href_seq_of_tax						= $_[0] ;	# key: taxon; value: corresponding sequences											-> IN (defined) / OUT (unchanged)
		...
		my $sref_inputfile						= $_[4] ;	# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
		my $sref_prottest_name					= $_[5] ;	# Outfilename of output option															-> IN (defined) / OUT (unchanged)
		...
		unless	( $$sref_file_parameter =~ /Supermatrix/ ){ print "\n\t!COMMAND-ERROR!: ProtTest not started due to unspecified concatenation process!" }
			...
			############################################################ START HANDLING OF AA DATA
				...
				my $Ntaxa	= keys %$href_seq_of_tax ;
				my $Nchar	= $href_hoh_info_of_infile_of_type->{$infile}{seqlength} ;
				...
				for my $taxon ( keys %$href_seq_of_tax ){
					...
					( my $taxon_new	= $taxon ) =~ s/ /_/g;
					
					print OUTphy $taxon_new, "   ", $href_seq_of_tax->{$taxon}, "\n"
				}
				##############################
	...
	sub seq_renaming{
		...
		my $href_seq_of_tax						= $_[0] ;	# key: taxon; value: corresponding sequences											-> IN (undefined) / OUT (defined)
		...
		my $sref_inputfile						= $_[3] ;	# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
		my $sref_structure						= $_[4] ;	# defined if structure sequence present in infiles										-> IN (defined) / OUT (changed)
		...
		for my $taxon ( sort keys %$href_seq_of_tax ){
			
			if ( $new_name_of_old_name{$taxon} ){
				
				$sequence_of_new_names{$new_name_of_old_name{$taxon}} = $href_seq_of_tax->{$taxon} ;
				$counter_changed++ ;
				...
				if ( $taxon eq $$sref_structure ){
					...
					$href_hoh_info_of_infile_of_type->{$$sref_inputfile}{taxstruct} = $$sref_structure
				}
				##############################
			}
				...
				$sequence_of_new_names{$taxon} = $href_seq_of_tax->{$taxon} ;
				$counter_unchanged++;
		...
		%$href_seq_of_tax = %sequence_of_new_names ;
		
		for my $taxon ( keys %sequence_of_new_names ){ $href_name_list->{$taxon}++ }
	...
	sub tax_sampling{
		...
		my $href_taxa_presampled				= $_[0] ;	# sampled taxon names for sequence concatenation										-> IN (undefined) / OUT (defined)
		my $aref_inputfile_list					= $_[1] ;	# list of input files found in fas, FASTA, phy, or aln format							-> IN (defined) / OUT (unchanged)
		...
		############################################################ START TAXON SAMPLING
		## READ IN infiles and sample taxon names
		for my $file ( sort @$aref_inputfile_list ){
			...
			##############################
			## Sampling Taxon Names
			for my $taxon ( sort keys %sequence_of_taxon ){ $href_taxa_presampled->{$taxon}++ }
			##############################
		}
		
		print "\n\n\tSampled taxon names:" ;
		for my $taxon ( sort keys %$href_taxa_presampled ){ print "\n\t", $taxon, "\t", $href_taxa_presampled->{$taxon}, " file(s)" }
		############################################################ END TAXON SAMPLING	
	}
	...
	sub input_check{
		...
		my $sref_inputfile						= $_[2] ;	# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
		...
		############################################################ START FILE CHECK
		## READ IN infile, check for correct format, and make consensus sequence blocks if defined
		...
		## %sequence_of_taxon ->  # key: taxon; value sequence of taxon ;
		my %sequence_of_taxon ;
		if ( $$sref_inputfile =~ /^.*\.aln$/i             ){ &aln2fas ( \$$sref_inputfile, \%sequence_of_taxon ) }	# READ IN CLUSTAL files (.aln)
		if ( $$sref_inputfile =~ /^.*\.phy$/i             ){ &phy2fas ( \$$sref_inputfile, \%sequence_of_taxon ) }	# READ IN PHYLIP  files (.phy)
		if ( $$sref_inputfile =~ /^.*\.fas$|^.*\.fasta$/i ){ &fasEDIT ( \$$sref_inputfile, \%sequence_of_taxon ) }	# READ IN FASTA   files (.fas || .FASTA)
		##############################
		...
		my	%counter_taxa_of_file ;	# counts specified taxon names in infile. If a taxon name occurs more than once, FASconCAT dies with an error prompt
			$href_hoh_info_of_infile_of_type->{$$sref_inputfile}{seqtype} = 'nu' ;	# sequence type predefined as nucleotide data set. If amino acid data unique character states are found, sequence type will be changed to 'aa'
		##############################
		...
		for my $taxon ( sort keys %sequence_of_taxon ){
			...
			unless ( $href_hoh_info_of_infile_of_type->{$$sref_inputfile}{seqlength} ){ $href_hoh_info_of_infile_of_type->{$$sref_inputfile}{seqlength} = length $sequence_of_taxon{$taxon} }
			##############################
			...
			die    "\n\t!FILE-ERROR!: Unequal sequence lengths in file ", $$sref_inputfile, "!\n"							if length	$sequence_of_taxon{$taxon}	!= $href_hoh_info_of_infile_of_type->{$$sref_inputfile}{seqlength} ;	# (2)
			die    "\n\t!FILE-ERROR!: Sequence missing for taxon ", $taxon, " in file ", $$sref_inputfile, "!\n"			unless		$sequence_of_taxon{$taxon}	;						# (3)
			die    "\n\t!FILE-ERROR!: Taxon ", $taxon, " appears multiple times in file ", $$sref_inputfile, "!\n"			if			$counter_taxa_of_file{$taxon} > 1 ;					# (4)
			##############################
			...
			if ( $sequence_of_taxon{$taxon} =~ /.*\(.*\).*/ ){
				...
				unless ( $href_hoh_info_of_infile_of_type->{$$sref_inputfile}{taxstruct} ){ $href_hoh_info_of_infile_of_type->{$$sref_inputfile}{taxstruct} = $taxon ; $$sref_structure = $taxon }
				##############################
				...
				for my $str_sign ( @strc_elements ){
					...
					unless ( $str_sign =~ /\(|\)|\./ ){ die "\n\t!FILE-ERROR!: Unallowed signs in structure sequence ", $taxon, " of file ", $$sref_inputfile, "!\n" }	# (1)
				}
				...
				if ( $href_hoh_info_of_infile_of_type->{$$sref_inputfile}{taxstruct} ne $taxon ){ die "\n\t!FILE-ERROR!: Multiple structure sequences found: ", $href_hoh_info_of_infile_of_type->{$$sref_inputfile}{taxstruct}, ", ", $taxon, "!\n" }	# (2)
				else { print  "\n\t!FILE-INFO!: Structure sequence ", $taxon, " found in file ", $$sref_inputfile, "!\n" }
				##############################
			}
			...
			## (2) ...sequence type (nucleotide -> 'nu' is predefined. If amino acid unique sequence are found set sequence type of infile to amino acid -> 'aa'
				...
				for my $seq_sign ( @seq_elements ){ 
					...
					unless ( $seq_sign =~ /A|C|G|T|U|-|N|Y|X|R|W|S|K|M|D|V|H|B|Q|E|I|L|F|P|\?/ ){ die "\n\t!FILE-ERROR!: Unknown character found in sequence ", $taxon, " of file ", $$sref_inputfile, "!\n" }	# (1)
				}
				...
				if ( $sequence_of_taxon{$taxon}  =~ /I|E|L|Q|F|P/i ) { $href_hoh_info_of_infile_of_type->{$$sref_inputfile}{seqtype} = 'aa' } # (2)
				##############################
			}
			...
			$href_seq_of_tax->{$taxon} = $sequence_of_taxon{$taxon} ; # (1)
			$href_taxa_all->{$taxon}++								; # (2)
			$href_hoh_info_of_infile_of_type->{$$sref_inputfile}{Ntaxa}++		; # (3)
			##############################
		}
		...
		if 		( $href_hoh_info_of_infile_of_type->{$$sref_inputfile}{seqtype} eq 'aa' ){ print "\n\t!FILE-INFO!: Infile ", $$sref_inputfile, " identified as AA data set !\n" }
		elsif	( $href_hoh_info_of_infile_of_type->{$$sref_inputfile}{seqtype} eq 'nu' ){ print "\n\t!FILE-INFO!: Infile ", $$sref_inputfile, " identified as NUC data set !\n" }
		else 	{ die "\n\t!BUG-ERROR!: Cannot assign sequence type of infile ", $$sref_inputfile, " in subroutine &input_check!\n\tPlease, report BUG to system developer!" }
		##############################
		############################################################ END FILE CHECK
		...
		sub phy2fas{
			...
			for my $line_c ( sort {$a<=>$b} keys %seq_phy ){ 
				...
				my	@seq_single_parts						=	exists($seq_phy{$line_c}) ? @{$seq_phy{$line_c}} :( ) ;
					$seq_single_parts[0]					=~	s/\s+/ / ;	# reduce multiple blanks betweenn taxon name and sequence begin to one
					$seq_single_parts[@seq_single_parts-1]	=~	s/\s+$// ;	# exclude posiible blanks at the end of the sequence line
				my	$seq_complete							=	join	""		, @seq_single_parts ; #print "\n\ncomplete:", $seq_complete, "\n" ;
					@seq_single_parts						=	split	" "		, $seq_complete ;
				
				if ( @seq_single_parts == 2 ){ 
					
					my	$taxon								=	$seq_single_parts[0] ; #print "\ntaxon:", $taxon, "\nsequence:", $seq_single_parts[1] ;
					...
					$href_seq_of_tax->{$taxon}	=	$seq_single_parts[1] ;
				}
			}
			########################################################################
		}
	...
	sub seq_translation{
		...
		my $href_hoh_info_of_infile_of_type		= $_[1] ;	# key1: infilename, key2: type of info (e.g. 'seqlength'); info type associated value	-> IN (defined) / OUT (changed)
		my $sref_inputfile						= $_[2] ;	# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
		my $sref_tra_parameter					= $_[3] ;	# defined ry coding parameter option													-> IN (defined) / OUT (unchanged)
		...
		my %sequence_of_taxon = %$href_seq_of_tax ;
		##############################
		...
		%$href_seq_of_tax  = %sequence_of_taxon ;
		%sequence_of_taxon = () ;
		##############################
	...
	sub ry_coding{
		...
		my $href_hoh_info_of_infile_of_type		= $_[1] ;	# key1: infilename, key2: type of info (e.g. 'seqlength'); info type associated value	-> IN (defined) / OUT (unchanged)
		my $sref_inputfile						= $_[2] ;	# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
		my $sref_ry_parameter					= $_[3] ;	# defined ry coding parameter option													-> IN (defined) / OUT (unchanged)
		...
		############################################################ START RY CODING OF Actual INFILE
		my $infile = $$sref_inputfile ;
		...
		if ( $href_hoh_info_of_infile_of_type->{$infile}{seqtype} eq 'nu' ){
			...
			##############################
			## assign recoded sequences to %$href_hol_seq_of_tax_of_infile
			%$href_seq_of_tax  = %RY_code_of_taxon ;
			%RY_code_of_taxon  = () ;
			%sequence_of_taxon = () ;
			##############################
		}
	...
	sub reject_third_nuc_position{
		...
		my $href_seq_of_tax						= $_[0] ;	# key: taxon; value: corresponding sequences											-> IN (undefined) / OUT (defined)
		my $sref_inputfile						= $_[1] ;	# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
		...
		if ( $href_hoh_info_of_infile_of_type->{$infile}{seqtype} eq 'nu' ){
			...
			for my $taxon ( sort keys %sequence_of_taxon ){ 
				...
				$href_hoh_info_of_infile_of_type->{$infile}{seqlength} = length $sequence_of_taxon{$taxon} 
			}
	...
	sub concatenate{
		...
		my $href_concstring_of_taxon			= $_[4] ;	# key: taxon; value: concatenated string												-> IN (defined) / OUT (changed)
		my $sref_inputfile						= $_[5] ;	# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
		my $sref_fill_code						= $_[6] ;	# defined replacement parameter (MISSING or INDEL)										-> IN (defined) / OUT (unchanged)
		...
		unless (  $href_hoh_info_of_infile_of_type->{supermatrix}{seqstart} ){
		...
		for my $taxon ( sort {$a<=>$b} keys %$href_name_list ){
			...
			if 		( $sequence_of_taxon{$taxon} 											){ $href_concstring_of_taxon->{$taxon} .= $sequence_of_taxon{$taxon} 	; $href_hoh_info_of_infile_of_type->{supermatrix}{Nseqconcat  }{$taxon}++	; $href_hoh_info_of_infile_of_type->{$infile}{Ntaxa_in}++		}
			elsif	( $taxon	eq $$sref_structure											){ $href_concstring_of_taxon->{$taxon} .= $string_missing_taxon_dots	; $href_hoh_info_of_infile_of_type->{supermatrix}{Nsequnconcat}{$taxon}++	; $href_hoh_info_of_infile_of_type->{$infile}{Ntaxa_mis}++	} #push @{$href_hol_seq_of_tax_of_infile->{$infile}}, ( $taxon, $string_missing_taxon_dots	) }	# (1)
			elsif	( 'nu'		eq $href_hoh_info_of_infile_of_type->{$infile}{seqtype  }	){ $href_concstring_of_taxon->{$taxon} .= $string_missing_taxon_states	; $href_hoh_info_of_infile_of_type->{supermatrix}{Nsequnconcat}{$taxon}++	; $href_hoh_info_of_infile_of_type->{$infile}{Ntaxa_mis}++	} #push @{$href_hol_seq_of_tax_of_infile->{$infile}}, ( $taxon, $string_missing_taxon_states	) }	# (2)
			...
			$href_hoh_info_of_infile_of_type->{supermatrix}{seqlength} = length $href_concstring_of_taxon->{$taxon} ;
		}
	...
	sub make_consensus{
		...
		my $sref_inputfile						= $_[3] ;	# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
		my $href_name_list						= $_[4] ;	# key: taxon name; value : number of occurence in infiles								-> IN (defined) / OUT (changed)
		my $href_taxa_presampled				= $_[5] ;	# sampled taxon names for sequence concatenation										-> IN (defined) / OUT (changed)
		...
		for my $taxon ( sort keys %sequence_of_taxon ){
			...
			unless ( $taxon eq $href_hoh_info_of_infile_of_type->{$infile}{taxstruct} ){
				...
				unlink $href_taxa_presampled->{$taxon} ;
				$href_taxa_presampled->{$con_taxon}++ ;
		...
		for my $con_taxon ( sort keys %consensus_seq_of_taxon ){
			...
			$href_seq_of_tax->{$con_taxon} = $consensus_seq_of_taxon{$con_taxon} ;
	...
	sub print_out{
		...
		my $sref_inputfile						= $_[19];	# input file found in fas, FASTA, phy, or aln format									-> IN (defined) / OUT (unchanged)
		my $href_name_list						= $_[20] ;	# key: taxon name; value : counter														-> IN (defined) / OUT (unchanged)
		my $sref_printflag						= $_[21];	# flag for file printing																-> IN (defined) / OUT (unchanged)


		############################################################ START SUPERMATRIX PRINT OUT
		if ( $$sref_printflag eq 'datafile'){
			
			##############################
			## define output file names according to chosen infile handling option
			my @outfiles = $$sref_inputfile ;
			##############################
			
			##############################
			## set flag to avoid multiple raxml partition prints
			my $flag_partfile = 1 ;
			##############################
			## OUTPUT -> FASTA format 
			unless ( $aref_parameter_fas->[0] eq 'NO' ){
				...
				&fas_out(
							...
							\%$href_seq_of_tax,					# key: taxon; value ; corresponding sequence											-> IN (defined) / OUT (unchanged)
				...
				if ( ( $aref_parameter_prt->[0] eq 'Supermatrix' ) && ( $outfiles[0] =~ /supermatrix/ ) ){
			...
			unless ( $aref_parameter_phy->[0] eq 'NO' ){
				...
				if ( ( $aref_parameter_prt->[0] eq 'Supermatrix' ) && ( $outfiles[0] =~ /supermatrix/ ) && ( $flag_partfile == 1 ) ){
		...
		############################################################ START INFO PRINT OUT
			...
			if ( $aref_parameter_inf->[0] eq 'NO' ){
				...
				&info_small (
							\%$href_name_list,						# key: taxon; value: counter															-> IN (defined) / OUT (unchanged)
							\%$href_hoh_info_of_infile_of_type,		# key1: infilename, key2: type of info (e.g. 'seqlength'); info type associated value	-> IN (defined) / OUT (unchanged)
		...
		sub info_small{
			...
			my $href_name_all					= $_[0] ;	# key: taxon; value: counter															-> IN (defined) / OUT (unchanged)
			my $href_hoh_info_of_infile_of_type	= $_[1] ;	# key1: infilename, key2: type of info (e.g. 'seqlength'); info type associated value	-> IN (defined) / OUT (unchanged)
			...
			if ( $aref_parameter_fil->[0] =~ /Supermatrix/ ){
				...
				for my $infile ( sort keys %$href_hoh_info_of_infile_of_type ){
			...
			if ( $aref_parameter_fil->[0] =~ /Convert/ ){
				...
				print OUT_info "\n\nSingle Infile(s)\tL SEQUENCE [bp]\tCONVERTED TO FASTA\tCONVERTED TO NEXUS\tCONVERTED TO PHYLIP" ;
				
				for my $infile ( sort {$a<=>$b} keys %$href_hoh_info_of_infile_of_type ){
			...
			if ( $aref_parameter_fil->[0] =~ /Supermatrix/ ){
				...
				for my $taxon ( sort {$a<=>$b} keys %$href_name_all ){
		...
		sub info_all{
			...
			if ( $$sref_structure_seq ){
				...
				if ( $aref_parameter_fil->[0] =~ /Convert/ ){
					...
					##############################
					## (1) ...define all infiles for print of extra structure info in separate outfile
					for my $infile ( sort {$a<=>$b} keys %$href_hoh_info_of_infile_of_type ){ if ( $href_hoh_info_of_infile_of_type->{$infile}{taxstruct} ){ push @outfiles, $infile unless $infile eq 'supermatrix'} }
					##############################
		...
		sub fas_out{
			...
			my $href_seq_of_tax					= $_[1] ;	# key: taxon; value ; corresponding sequence											-> IN (defined) / OUT (unchanged)
			my $href_hoh_info_of_infile_of_type	= $_[2] ;	# key1: infilename, key2: type of info (e.g. 'seqlength'); info type associated value	-> IN (defined) / OUT (changed)
		...
		sub nexus_out{
			...
			my $href_seq_of_tax						= $_[2] ;	# key: taxon; value ; corresponding sequence											-> IN (defined) / OUT (unchanged)
			...
			for my $file ( @$aref_outfile_names ){
				...
				my %sequence_of_taxon	= %$href_seq_of_tax										;	# (1)
				my $sequence_length		= $href_hoh_info_of_infile_of_type->{$file}{seqlength} 	;	# (2)
				...
				for my $infile ( sort {$a<=>$b} keys %$href_hoh_info_of_infile_of_type ){
					...
					unless ( $infile eq 'supermatrix' ){
						...
						for my $taxon ( sort keys %$href_seq_of_tax ){
							...
							my $seq_substring = substr( $href_seq_of_tax->{$taxon}, $startposition-1, $length ) ;
		...
		sub phylip_out{
			...
			for my $file ( @$aref_outfile_names ){
				...
				## ---------------
				my $Ntaxa	= keys %$href_seq_of_tax ;
				my $Nchar	= $href_hoh_info_of_infile_of_type->{$file}{seqlength} ;
				...
				## ---------------
					...
					for my $taxon ( keys %$href_seq_of_tax ){	# (1)
						...
						if ( $Nsigns_taxon > 10 ){
							...
							print OUTphy $taxon_restricted, "   ", $href_seq_of_tax->{$taxon}, "\n"
							##############################
						}
						############################################################ END HANDLING OF TAXON NAMES GREATR 10 SIGNS
							...
							##############################
							# PRINT OUT taxon and corresponding sequence
							print OUTphy $taxon, "   ", $href_seq_of_tax->{$taxon}, "\n"
							##############################
						}
					}
		...
		sub parsimony_print{
			...
			for my $file ( @$aref_outfile_names ){
				...
				for my $taxon ( keys %$href_seq_of_tax ){
					...
					my @sites = split "", $href_seq_of_tax->{$taxon} ;
			...
			unless ( $$sref_parameter_fas eq 'NO' ){
				...
				&fas_out(
							...
							\%$href_seq_of_tax,						# key: input file name; value ; list of taxa and corresponding sequences				-> IN (defined) / OUT (unchanged)
							\%$href_hoh_info_of_infile_of_type,		# key1: infilename, key2: type of info (e.g. 'seqlength'); info type associated value	-> IN (defined) / OUT (unchanged)
