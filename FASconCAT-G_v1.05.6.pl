...
# written by patrick kueck, zentrales forschungsmuseum alexander koenig, bonn, germany
# email: patrick_kueck[at]web[dot]de
...
sub argv_handling{
    ...
    if ( $commandline ){
        ...
        for my $single_command ( sort @commands ){
            ...
            elsif     ( $single_command =~ /^g$/i    ) { @$aref_parameter_mis = reverse @$aref_parameter_mis }                  # '-g'    -> replace missing sequences by '-';             '-g -g' -> replace missing sequences by 'X' or 'N' (default) #jgl#'-x -> -g'
...
#sub menu{ system('cls'); #jgl# only clears screen in Windows environment
sub menu {
    ...
    print "";
    printf "\n%68s\n","------------------------------------------------------------";
    printf "%53s\n"  ,"Welcome to FASconCAT-G v$script_version !"                   ;
...
sub parameter{
    ...
    print    "\tSTART\t    FASconCAT      :\t\t type <s> <enter>",
            ...
            "\n\tPREFACE     FASconCAT      :\t\t type <x> <enter>", #jgl#'<g> -> <x>'
            "\n",
            "\n\t------------------------------------------------------------",
            "\n",
            "\n\tFILE INPUT",
    ...
    ##############################
    ## (1) Read IN user defined command #jgl#'=> d'
    ...
    elsif ( $single_command =~ /^x$/i ) { &preface}                                                              # '-x' -> open preface menu #jgl#'/^g> -> /^x'; '-e -> -x'
...
sub help{
    ...
    #system('cls'); #jgl# only clears screen in Windows environment
    if ($^O eq 'MSWin32') { system('cls'); }
    else { system('clear'); }
...
<<help
    ...
    - Open the menu by entering command:
         perl FASconCAT-G_v$script_version.pl <enter> (Linux/Mac)
              FASconCAT-G_v$script_version.pl <enter> (Windows)
    - To start FASconCAT under default, enter command:
         s <enter>

    >Via terminal command line, enter command:
    - perl FASconCAT_v$script_version.pl [command options] <enter> (Linux/Mac)
    -      FASconCAT_v$script_version.pl [command options] <enter> (Windows)
    - To start FASconCAT under default, enter command:
         perl FASconCAT_v$script_version.pl -s <enter>
    --------------------------
    ...
    the manual or write an email to patrick_kueck[at]web[dot]de
    ------------------------------------------------------------
...
<<preface
    ...
    e-mail      : patrick_kueck[at]web[dot]de
...
sub start{
    ...
    ##############################
    ## Definitions of global variables used in this subroutine and those that follow
    my (
        ...
        unless    ( $$sref_file_parameter =~ /Supermatrix/ ){ print "\n\t!COMMAND-ERROR!: ProtTest not started due to unspecified concatenation process!" }
            ...
            for my $infile ( sort {$a<=>$b} keys %$href_hol_seq_of_tax_of_infile ){
                ...
                ############################################################ START HANDLING OF AA DATA
                    ...
                    ##############################
                    ## print file handling info
                    ( my $file = $infile ) =~ s/.fas$|.fasta$|.FAS$|.FASTA$|.phy$|.aln$//;
                    print "\n\t...for ", $file, "\n\n"; 
                    ##############################
                    ...
                    ##############################
                    ## START PROTTEST
                    #( my $file = $infile ) =~ s/.fas$|.FAS$|.FASTA$|.phy$|.aln$|.fasta$//; #jgl# already defined above {## print file handling info}
    ...
    sub input_check{
        ...
        for my $file ( sort @$aref_inputfile_list ){
            ...
            for my $taxon ( sort keys %sequence_of_taxon ){
                ...
                unless ( $href_hoh_info_of_infile_of_type->{$file}{seqlength} ){
                    $href_hoh_info_of_infile_of_type->{$file}{seqlength} = length $sequence_of_taxon{$taxon};
                }
                ##############################
        ...
        sub aln2fas{
            ...
            #jgl# Warning >Useless use of private variable in void context<
            #-----------------------------------------------------------
        ...
        for my $infile ( sort {$a<=>$b} keys %$href_hol_seq_of_tax_of_infile ){
            ...
            if ( ( $href_hoh_info_of_infile_of_type->{$infile}{seqtype} eq 'nu' ) && ( $$sref_tra_parameter eq 'NUC to AA' ) ){
                ...
                unless ( $href_hoh_info_of_infile_of_type->{$infile}{taxstruct} ){
                    ...
                    ##############################
                    ## For each taxon start triplet substitution to amino acid state
    ...
    sub reject_third_nuc_position{
        ...
        for my $infile ( sort {$a<=>$b} keys %$href_hol_seq_of_tax_of_infile ){
            ...
            if ( $href_hoh_info_of_infile_of_type->{$infile}{seqtype} eq 'nu' ){
                ...
                for my $taxon ( sort keys %sequence_of_taxon ){
                    push @{$href_hol_seq_of_tax_of_infile->{$infile}}, ( $taxon, $sequence_of_taxon{$taxon} );
    ...
    sub concatenate{
        ...
        for my $infile ( sort keys %$href_hol_seq_of_tax_of_infile ){
            ...
            ## (2) ... indels ('-') if specified via -g option, otherwise with
            ...
            if ( $href_hoh_info_of_infile_of_type->{$infile}{taxstruct} ){
                ...
                for ( @structures ){ $i++;
                    ...
                    if    ( $_  =~ /\(/ ){ push @forward, $i } # (1)
                        ...
                        my $pair_1 = pop @forward;
                        push @pairs  , ( $pair_1.":".$i );
                        push @pairs_list,( $pair_1, $i )
                    }                                          # (2)
    ...
    sub print_out{
        ...
        sub fas_out{
            ...
            for my $file ( @$aref_outfile_names ){
                ...
                #close OUT_smatrix #jgl# wrong fh
                close OUT_fas
            }
...
sub commandline{
    ...
    #my $sref_print_text = $_[0]; #jgl# uninitialzed scalar ref?


    ##############################
    ## print text
    #print "\n\t", $$sref_print_text, #jgl# uninitialzed scalar ref?
    print "\n\tCOMMAND:\t ";
    ##############################
