...
sub start{
	...
	sub single_define{
		...
		my $aref_parameter_pro	= $_[13];	# Start prottest analyses for aa data							-> IN (defined) / OUT (unchanged)
		my $href_outfile_name	= $_[14];	# Outfilename of output option									-> IN (defined) / OUT (unchanged)
		my $aref_input_files	= $_[15];	# list of input files found in fas, FASTA, phy, or aln format	-> IN (defined) / OUT (changed)
	...
	sub concatenate{
		...
		for my $infile ( sort keys %$href_hol_seq_of_tax_of_infile ){
			...
			## (2) ... N's if infile sequences consist of nucleotide sequence
			## (3) ... X's if infile sequences consist of amino acid sequence
			##
			...
			if ( $href_hoh_info_of_infile_of_type->{$infile}{seqtype} eq 'nu' 		)	{ $string_missing_taxon_states	= ( "N" x $href_hoh_info_of_infile_of_type->{$infile}{seqlength} ) }
	...
	sub print_out{
		...
		my $sref_structure_seq					= $_[17];	# defined if structure sequence present in infiles										-> IN (defined) / OUT (unchanged)
		...
		sub info_all{
			...
			my $structure_seq							= $_[18];	# defined if structure sequence present in infiles										-> IN (defined) / OUT (unchanged)
	...
	sub end{
		...
		my $aref_parameter_ren	= $_[11];	# Rename taxon names of given ifiles					-> IN (defined) / OUT (unchanged)
		my $aref_parameter_prt	= $_[12];	# Print partition files for concatenated data			-> IN (defined) / OUT (unchanged)
		my $sref_structure_seq	= $_[13];	# defined if structure sequence present in infiles		-> IN (defined) / OUT (unchanged)
